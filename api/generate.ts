...
const AVATAR_VOICE_MAP: Record<string, string> = {
  ...
  // ✅ New avatars added
  "b6529e10fb6a45aabe730acff799aebf": "voice_id_here", // Prashant R | JFS Avatar
  "38ab20bc42634d368d4072b102aaa3d9": "voice_id_here", // Anoushka Chauhan
  "3024995942d148c887c9df208444c663": "voice_id_here", // Garvik
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      delete fallbackPayload.avatar_version;
    ...
    if (!response.ok) {
      throw new Error(`HeyGen ${response.status}: ${responseText}`);
    }
    ...
    if (!video_id) {
      throw new Error(`No video_id returned: ${responseText}`);
    }
