...
// ============================================================
// Correct display names (HeyGen API returns wrong names)
// ============================================================
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // ── Filter to personal avatars only + fix names + dedupe ─
    const seen = new Set<string>();
    const myAvatars = allAvatars
    ...
    console.log(`✅ Returning ${myAvatars.length}/${MY_AVATAR_IDS.size} avatars`);
    return res.status(200).json(myAvatars);
